...
public class OscMessage : OscPacket
{
	...
	public void RemoveAt( int index )
	{
		...
		if( argToRemoveInfo.byteCount > 0 )
		{
			...
			_argsByteCount -= argToRemoveInfo.byteCount;

			// Update byte indices.
			int byteIndex = 0;
			if( index > 0 ) {
				OscArgInfo argInfo = _argsInfo[ index-1 ];
				byteIndex = argInfo.byteIndex + argInfo.byteCount;
			}
	...
	public bool TryGetArgType( int index, out OscArgType type )
	{
		...
		type = OscConverter.ToArgType( _argsInfo[index].tagByte );
		return true;
	}
	...
	public static bool TryReadFrom( byte[] sourceData, ref int sourceIndex, ref OscMessage message )
	{
		...
		//Debug.Log( $"Receiving { string.Join( ",", sourceData ) }" );
		...
		for( int i = 0; i < argCount; i++ )
		{
			...
			message._argsInfo[i] = new OscArgInfo( tagByte, argByteCount, newArgsByteCount );
	...
	int AdaptiveSet( int index, OscArgInfo newArginfo )
	{
		...
		if( expandArgs ) newArginfo.byteIndex = oldArgsByteCount;
	...
	bool ValidateTryGet( int index, OscArgType requestedType )
	{
		...
		if( index < 0 || index >= _argsCount ) {
			if( OscGlobals.logWarnings ) {
				...
				sb.Append( " is out of bounds. Message has " ); sb.Append( _argsCount );
				sb.Append( " arguments.\n" );
